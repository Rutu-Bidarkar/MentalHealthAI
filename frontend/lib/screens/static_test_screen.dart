...
class _StaticTestScreenState extends State<StaticTestScreen> {
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: _answers.length < widget.questions.length
                    ? null
                    : () {
                        int totalScore = _answers.values.fold(
                          0,
                          (sum, val) => sum + (val ?? 0),
                        );
      ...
      child: Column(
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              fontSize: 10,
              color: isSelected ? const Color(0xFF2AB5B4) : Colors.grey,
            ),
          ),
        ],
      ),
  ...
  void _showResults(int score) {
    ...
    // ✅ Fix 2, 3, 4, 5: Added curly braces to all if/else branches
    if (score < 10) {
      interpretation = "Minimal concern";
    } else if (score < 20) {
      interpretation = "Mild concern";
    } else if (score < 30) {
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            const Text(
              "⚠️ Reminder: This is not a clinical diagnosis. Please consult a professional.",
            ),
          ],
        ),
        actions: [
