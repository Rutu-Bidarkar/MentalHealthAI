...
class QuickAccessTab {
  ...
  const QuickAccessTab({
    ...
    required this.label,
    required this.path,
    required this.gradient, required String imagePath,
  });
}
    ...
    return Row(
      ...
      children: [
        ...
        Row(
          children: [
            IconButton(
                icon: const Icon(Icons.bar_chart), onPressed: _addToReport),
            IconButton(
                icon: const Icon(Icons.delete_outline), onPressed: _clearJar),
          ],
        )
      ],
    );
  ...
  Widget _buildJarVisual() {
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          Image.asset('assets/images/jar.png',
              width: 280, fit: BoxFit.contain),
          ...collectedMarbles.map((marble) => Positioned(
                left: marble.x,
                top: marble.y,
                child: Image.asset(
                  marble.mood.imagePath,
                  width: 32,
                  height: 32,
                ),
              )),
...
class _QuickAccessCard extends StatelessWidget {
  ...
  const _QuickAccessCard({Key? key, required this.tab}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () => Navigator.pushNamed(context, tab.path),
      borderRadius: BorderRadius.circular(AppTheme.borderRadius),
      child: Container(
        decoration: BoxDecoration(
